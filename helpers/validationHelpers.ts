...
type IValidationFunctionsReturns = {
  isValidStatus: boolean;
  stateInfo: string | number;
};
export type IValidationFunctions = {
  [key in keyof typeof DefaultValidations]: (
    ...args: any[]
  ) => IValidationFunctionsReturns;
};
...
const validationFunctions: IValidationFunctions = {
  ...
  email(value: string): IValidationFunctionsReturns {
    return {
      isValidStatus:
        value.length !== 0 && !!emailRegExp.test(String(value).toLowerCase()),
      stateInfo: "",
    };
  },
  ...
  phone(value: string): IValidationFunctionsReturns {
    return {
      isValidStatus: value.length !== 0 && value.length === 10,
      stateInfo: "",
    };
  },
  fileSize(file: File): IValidationFunctionsReturns {
    return {
      isValidStatus: file.size < 10000000 && file.size > 0,
      stateInfo: "",
    };
  },
...
export function testField<T extends IFormFieldType>(
  ...
  if (typeof field === "boolean")
    validationMethods = validationFunctionsForBoolean;
  else if (typeof field === "string")
    validationMethods = validationFunctionsForString;
  else if (field instanceof File)
    validationMethods = validationFunctionsForFile;
  else {
  ...
  for (let item in rule) {
    if (validationMethods[item])
      // @ts-ignore - ts борется с изменение типов параметров цикла item,index
      errorsObject[item] = validationMethods[item](field, rule[item])
        .isValidStatus
        ? errorsObject[item]
        : // @ts-ignore - ts борется с изменение типов параметров цикла item,index
          ValidateTextError[item] +
          // @ts-ignore - ts борется с изменение типов параметров цикла item,index
          validationMethods[item](field, rule[item]).stateInfo;
  }
...
// запуск этого метода проходит по всем переданным валидируемым полям форм и возвращает false, если хоть 1 элемент формы невалидый
export const validateForm = (validatable: IValidatableComponent) => {
  ...
  Object.entries(validatable.form).forEach((entry) => {
    ...
    validatable.rules[key].state.errorsObject = testField(
      value,
      validatable.rules[key].rules
    );

    if (Object.values(validatable.rules[key].state.errorsObject).join(""))
      isValid = false;
  });
